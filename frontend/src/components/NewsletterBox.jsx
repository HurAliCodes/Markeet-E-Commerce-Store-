import React from 'react'

const NewsletterBox = () => {

  const onSubmitHandler = (e) => {
    e.preventDefault()
  }

  return (
    <div className="text-center">
      <p className="text-2xl font-normal font-poppins! text-gray-800">Subscribe Now & get 20% Off</p>
      <p className='text-gray-400 mt-3'>Join our mailing list to get updates to your email inbox.</p>
      <form onSubmit={onSubmitHandler} className="w-full sm:w-1/2 flex item-center gap-3 mx-auto my-6 border pl-3">
        <input type="email" placeholder="Enter your email" className="w-full sm:flex-1 outline-none" />
        <button type="submit" className="bg-black text-white text-xs px-10 py-4 uppercase">Subscribe</button>
      </form>
    </div>
  )
}

export default NewsletterBox